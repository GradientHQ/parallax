...
class TestPagedKVIntegration(unittest.TestCase):
    ...
    def test_prefill_slot_mapping(self):
        ...
        keys_np = np.zeros((batch_size, max_len, self.num_kv_heads, self.head_dim), dtype=np.float32)
        values_np = np.zeros((batch_size, max_len, self.num_kv_heads, self.head_dim), dtype=np.float32)
        ...
        self.assertTrue(mx.allclose(cached_k_b1_0, expected_k_b1_0).item(), "Req2 Token 0 Key Mismatch")
        ...
        self.assertTrue(mx.allclose(cached_k_b1_16, expected_k_b1_16).item(), "Req2 Token 16 Key Mismatch (Cross Block)")
