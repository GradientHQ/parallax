...
class MLXModelLoader:
    ...
    def load(
        ...
        tokenizer = load_tokenizer(model_path, eos_token_ids=self.config.get("eos_token_id", None))

        architectures = self.config.get("architectures", None)
        ...
        try:
            ...
            model_args = model_args_class.from_dict(self.config)
        except (ImportError, AttributeError) as e:
            raise ValueError(f"Failed to load architecture for model_type '{model_type}'.") from e

        dtype = getattr(mx, self.config.get("torch_dtype", "bfloat16"))
        ...
        return model_shard, self.config, tokenizer
