...
// key_stride, input_num_heads, cache_num_heads, head_dim, block_size,
// num_layers, num_blocks (scalars)

device {{T}} *key_cache_mut = (device {{T}} *)key_cache;

uint3 gid = thread_position_in_grid;

int dim_idx = gid.x; // range [0, cache_num_heads * head_dim)
int token_idx = gid.y; // range [0, num_tokens)

if (dim_idx >= cache_num_heads * head_dim) return;

int head_idx = dim_idx / head_dim;
int d_idx = dim_idx % head_dim;
int src_head_idx = input_num_heads == 1 ? 0 : head_idx;
if (src_head_idx >= input_num_heads) return;
...
long src_idx = (long)token_idx * input_num_heads * head_dim +
               src_head_idx * head_dim +
               d_idx;

long k_block_stride = cache_num_heads * block_size * head_dim;
