...
class MACPrefillAdder:
    ...
    def add_chunked_req(self, chunked_req: Request) -> Optional[Request]:
        ...
        if self.cache_manager.prefix_cache is not None:
            _, matched_tokens = self.cache_manager.prefix_cache.match_prefix(
        ...
        if self.rem_chunk_tokens is None:
            truncated = False
            chunked_req_offset = len(chunked_req.origin_input_ids)
        else:
    ...
    def add_one_req(self, req: Request) -> AddReqResult:
        ...
        # align to page size
        extend_input_len = (
            (extend_input_len + self.page_size - 1) // self.page_size * self.page_size
        )
