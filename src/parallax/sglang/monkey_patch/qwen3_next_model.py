...
class Qwen3GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        projected_states_qkvz, projected_states_ba = self._forward_input_proj(hidden_states)
...
class Qwen3NextForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.config = config
        self.pp_group = get_pp_group()
        # logger.info(f"self.pp_group.ranks: {self.pp_group.ranks}")
        # logger.info(f"self.pp_group.rank: {self.pp_group.rank}")
        # logger.info(f"PP first - last: {self.pp_group.first_rank} - {self.pp_group.last_rank}")
        # logger.info(f"PP rank: {self.pp_group.rank}")
        # logger.info(f"PP is_first: {self.pp_group.is_first_rank}")
        # logger.info(f"PP is_last: {self.pp_group.is_last_rank}")
        # assert self.pp_group.is_first_rank and self.pp_group.is_last_rank
        self.quant_config = quant_config
        self.model = Qwen3NextModel(config, quant_config, prefix=add_prefix("model", prefix))
