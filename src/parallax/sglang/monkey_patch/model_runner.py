...
class ModelRunner:
    ...
    def model_specific_adjustment(self):
        ...
        if server_args.prefill_attention_backend is not None and (
            server_args.prefill_attention_backend == server_args.decode_attention_backend
        ):  # override the default attention backend
            server_args.attention_backend = server_args.prefill_attention_backend

        if getattr(self.model_config.hf_config, "dual_chunk_attention_config", None) is not None:
        ...
        if server_args.attention_backend is None:
            ...
            if not self.use_mla_backend:
                ...
                if is_hopper_with_cuda_12_3():
                    ...
                    if (head_num == 128 or head_num == 16) and self.spec_algorithm.is_none():
                        server_args.attention_backend = "aiter"
                    else:
        ...
        if server_args.attention_backend == "fa3" and server_args.kv_cache_dtype == "fp8_e5m2":
            logger.warning(
        ...
        #  For more details, see: https://github.com/sgl-project/sglang/issues/8616
            ...
            self.dp_size > 1 and is_sm100_supported() and server_args.attention_backend != "triton"
        ):
            logger.info(
        ...
        if server_args.enable_hierarchical_cache and server_args.hicache_io_backend == "kernel":
    ...
    def update_weights_from_disk(self, model_path: str, load_format: str) -> tuple[bool, str]:
    ...
    def get_weights_by_name(self, name: str, truncate_size: int = 100) -> Optional[torch.Tensor]:
        ...
        try:
            return self.model.get_weights_by_name(name, truncate_size, tp_size=self.tp_size)
        except Exception as e:
    ...
    def profile_max_num_token(self, total_gpu_memory: int):
        ...
        if self.is_hybrid_gdn:
            ...
            pp_mamba_layers = [
                i for i in all_mamba_layers if i >= start_layer_id and i < end_layer_id
            ]
    ...
    def set_num_token_hybrid(self):
        if "Llama4ForConditionalGeneration" in self.model_config.hf_config.architectures:
            temp_ratio = (
                1 - self.is_hybrid
    ...
    def init_memory_pool(
        ...
        if self.req_to_token_pool is None:
            ...
            if self.server_args.disaggregation_mode == "decode":
                ...
                pp_mamba_layers = [
                    i for i in mamba_layers if i >= start_layer_id and i < end_layer_id
                ]
    ...
    def init_double_sparsity_channel_config(self, selected_channel):
        ...
        for i in range(self.start_layer, self.end_layer):
            ...
            self.sorted_channels.append(
                torch.tensor(channel_config[key])[:, : self.server_args.ds_heavy_channel_num]
                .contiguous()
                .cuda()
            )
    ...
    def save_sharded_model(
        ...
        logger.info(f"Save sharded model to {path} with pattern {pattern} and max_size {max_size}")
